<template>
  <div>
    <a-spin tip="加载中 ···" :spinning="contentLoading">
      ...
      <a-card class="post-body" :bordered="false">
        <a-card-meta>
          ...
          <div class="post-body-description" slot="description">
            <!-- 富文本展示 -->
            <div style="color: #333333" v-html="postContent"></div>
            <!-- 用户上传的图片 -->
            <van-grid
      ...
      <!-- 回复面板 顶着键盘出来的 -->
      <van-overlay class="comment-overlay" :show="showInputComment" @click="showInputComment = false">
...
<script>
import contentCommentItem from "./content-s/content-comment-item.vue";
import backBall from "./dragBall-s/backBall.vue";
...
export default {
  ...
  created() {
    ...
    // 获取帖子详情信息
    reqwest({
      url: CONST.URL + "/discuss/detail/" + this.message.postID,
      type: "json",
      method: "GET",
      success: (res) => {
        this.postTitle = res.msg.post.title;
        this.postContent = res.msg.post.content;
        this.postPictures = res.msg.post.picUrls;
        this.postReleaseTime = CONST.getDiffTime(res.msg.post.createTime);
        ...
        this.postViews = res.msg.post.viewCount;
        this.postLikes = res.msg.post.likeCount;
        this.postLikeAction = res.msg.likeStatus === 1 ? "liked" : "disliked";

        this.userName = res.msg.user.username;
        this.userLevel = res.msg.user.level;
        this.userAvatar = res.msg.user.headerUrl;

        this.contentPostLoading = true
        this.$forceUpdate();
      },
    });
  },
  ...
  methods: {
    ...
    getPostComments() {
      ...
      reqwest({
        url: CONST.URL + "/comment/list/" + this.message.postID,
        type: "json",
        method: "GET",
        data: { current: 1 },
    ...
    likePost() {
      reqwest({
        url: CONST.URL + "/like?entityType=1&entityId="+this.message.postID +"&entityUserId="+this.message.userID+"&postId="+this.message.postID,
        type: "json",
        method: "POST",
        // contentType: "application/json",
        headers: {
        ...
        success: (res) => {
          ...
          this.postLikes = res.msg.likeCount;
          this.postLikeAction = res.msg.likeStatus === 1 ? "liked" : "disliked";
        },
      });
    },
