              ...
              >
                <a-card-meta>
                  <div style="font-size: 14px; color: #333" slot="description">
                    <div>我：{{ item.comment.content }}</div>
                  </div>
                </a-card-meta>
              </a-card>

              <div class="post" v-if="item.post != null" @click="goContent(item.post.id, item.post.userId)">
                <div style="color: #333333; font-size: 14px; font-weight: 600" slot="title">
                  {{ item.post.title }}
                  <van-tag 
                    style="font-weight: normal" color="#888" plain round
                   >原帖</van-tag
                  >
                  <span
                    style="font-size: 12px;font-weight: normal;margin-left: 5px;color: #888;"
                    >{{ CONST.getDiffTime(item.post.createTime) }}</span
                  >
                </div>
                <div id="my-post-content" class="post-content" style="font-size: 12px">
