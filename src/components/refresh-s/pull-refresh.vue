  ...
  >
    ...
    >
      <div class="loading" ref="loading" >
        <a-spin id='loading' class="loading-spin">
          <a-icon
            slot="indicator"
            type="redo"
...
export default {
  ...
  methods: {
    ...
    touchMove(e) {
      ...
      let newTop = (Math.max(Math.min(move,300),100)) / 2
      this.loading.style.top = newTop + 'px'
      this.loading.style.transform = 'rotate('+newTop*3+'deg)';
    ...
    touchEnd() {
      ...
      if(this.show){
        ...
        this.interval = setInterval(()=> { 
          this.loading.style.opacity  = CanSee
          CanSee = CanSee - 0.1
          if(CanSee<=0){
            clearInterval(this.interval)
            this.show = false;
